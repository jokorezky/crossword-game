...
function selectFirstWord() {
  if ($('#content').width() > 890 && !supportsTouch())
...
function resizeGrid(option) {
  ...
  if (option == RESIZE_OPTION.ZOOM_OUT) {
    ...
    if (canvasScale != 1) {
      ...
      var x =
        dragger.width() * canvasScale -
        dragger.width() -
        (board.width() - dragger.width());
      x = board.width() > dragger.width() * canvasScale ? 0 : x;

      var height = dragger.height() * canvasScale - dragger.height();
      var totalHeight =
        dragger.height() * canvasScale -
        dragger.height() +
...
function canvasMouseUp(ev) {
  ...
  var newY = dragger.height() * canvasScale - dragger.height();
  var height =
    dragger.height() * canvasScale -
    dragger.height() +
    (dragger.height() - board.height());
...
function setCanvasDimensions() {
  ...
  var boxWidth = parseInt(maxwidth);
...
function resize() {
  ...
  if (selectedCell) {
    if (selectedWord) selectClue(selectedWord, true);
...
function cellClicked(newCell) {
  ...
  if (screen.width >= 600) triggerInput(selectedCell);
  else {
...
function Cell(number, x, y) {
  ...
  this.update = function () {
    ...
    this.letterText.x =
      cellSize.width / 2 - this.letterText.getMeasuredWidth() / 2;
  };
